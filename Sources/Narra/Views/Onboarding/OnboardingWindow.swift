...
struct OnboardingWindow: View {
    ...
    enum Step: Int, CaseIterable {
        case welcome, provider, apiKey, hotkeys, mic, done

        /// Steps shown in the "Step n of 5" footer. `done` is the
    ...
    private var apiKeyStep: some View {
        ...
        return VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            HStack(spacing: Spacing.sm) {
                pillButton(title: "Save Key", systemImage: "key.fill", filled: true, disabled: apiKeyDraft.isEmpty) {
                    saveKey(for: provider)
                }
                pillButton(title: "Skip for now", systemImage: "forward.fill", filled: false, disabled: false) {
                    keySkipped = true
                }
                if keySaved {
                    HStack(spacing: 4) {
    ...
    private var doneStep: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            HStack {
                Spacer()
                pillButton(title: "Finish", systemImage: "checkmark", filled: true, disabled: false) {
        ...
        if step != .done {
            HStack(spacing: Spacing.md) {
                ...
                if step.rawValue > Step.welcome.rawValue {
                    pillButton(title: "Back", systemImage: "chevron.left", filled: false, disabled: false) {
                        retreat()
                    }
                }
    ...
    private func pillButton(title: String,
                            systemImage: String,
                            filled: Bool,
                            disabled: Bool,
        ...
        Button(action: action) {
            ...
            .foregroundStyle(filled ? Palette.canvas : Palette.ink)
            .padding(.horizontal, Spacing.md)
            .padding(.vertical, 6)
            .background(
                RoundedRectangle(cornerRadius: CornerRadius.md, style: .continuous)
                    .fill(filled ? Palette.ink : Color.white.opacity(0.05))
            )
            .overlay(
                RoundedRectangle(cornerRadius: CornerRadius.md, style: .continuous)
